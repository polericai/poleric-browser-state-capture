...
Output is Playwright-friendly JSON so it can be reused by automation runners.
"""
...
def resolve_bundle_paths(args: argparse.Namespace) -> dict[str, Path]:
    site_key = site_key_from_url(args.url)
    bundle_name = args.bundle or f"{site_key}-{args.mode}"
    bundle_dir = Path(args.output_dir).resolve() / bundle_name

    profile_dir = Path(args.profile_dir).resolve() if args.profile_dir else bundle_dir / "user_profile"

    return {
        "bundle_dir": bundle_dir,
        "storage_state": bundle_dir / "storage_state.json",
        "session_state": bundle_dir / "session_storage.json",
        "meta": bundle_dir / "meta.json",
        "verify_screenshot": bundle_dir / "verify.png",
...
def launch_context_and_page(playwright: Any, args: argparse.Namespace, paths: dict[str, Path]):
    ...
    session_path = paths["session_state"]

    browser_type = getattr(playwright, args.browser)

    reuse_storage = args.reuse_existing and storage_path.exists()
    storage_map_for_inject: dict[str, dict[str, str]] = {}
    cookies_for_add: list[dict[str, Any]] = []
    if reuse_storage:
        try:
            state_payload = read_json(storage_path)
            if isinstance(state_payload, dict):
                storage_map_for_inject = local_storage_map_from_state(state_payload)
                cookies = state_payload.get("cookies")
                if isinstance(cookies, list):
                    cookies_for_add = [c for c in cookies if isinstance(c, dict)]
        except Exception:
            storage_map_for_inject = {}
            cookies_for_add = []

    session_by_origin: dict[str, dict[str, str]] = {}
    if args.reuse_existing and session_path.exists():
        try:
            session_payload = read_json(session_path)
            session_by_origin = normalize_session_payload(session_payload)
        except Exception:
            session_by_origin = {}
    ...
    # Session storage is not part of Playwright storage_state, so inject it.
    if session_by_origin:
        context.add_init_script(script=build_storage_init_script(session_by_origin, "sessionStorage"))

    # If persistent profile is enabled, reuse cookies/localStorage from saved storage_state as well.
    if args.persistent_profile and reuse_storage:
        if cookies_for_add:
...
def capture_command(args: argparse.Namespace) -> int:
    ...
    print(f"[INFO] Bundle dir: {paths['bundle_dir']}")
    ...
    try:
        with sync_playwright() as playwright:
            ...
            # Save playwright storage state (cookies + localStorage + optional IndexedDB).
            try:
                state_payload = context.storage_state(path=str(paths["storage_state"]), indexed_db=True)
                indexed_db_saved = True
            except TypeError:
                state_payload = context.storage_state(path=str(paths["storage_state"]))
                indexed_db_saved = False

            session_entries: dict[str, dict[str, str]] = {}
            current_origin = f"{urlparse(page.url).scheme}://{urlparse(page.url).netloc}" if urlparse(page.url).scheme else ""
            if current_origin:
                raw_session = page.evaluate(SESSION_EXTRACT_JS)
                if isinstance(raw_session, dict) and raw_session:
                    session_entries[current_origin] = {
                        str(k): "" if v is None else str(v) for k, v in raw_session.items() if isinstance(k, str)
                    }
            ...
            write_json(
                paths["session_state"],
                {
                    ...
                    "by_origin": session_entries,
                },
            )

            cookies = state_payload.get("cookies", []) if isinstance(state_payload, dict) else []
            ...
            print(f"[DONE] Saved storage state: {paths['storage_state']}")
            print(f"[DONE] Saved session state: {paths['session_state']}")
            print(f"[DONE] Saved metadata: {paths['meta']}")
            return 0
...
def verify_command(args: argparse.Namespace) -> int:
    ...
    if not paths["storage_state"].exists():
        print(f"[ERROR] Missing storage_state.json: {paths['storage_state']}")
        return 1
...
def build_parser() -> argparse.ArgumentParser:
    ...
    def add_shared_args(cmd: argparse.ArgumentParser) -> None:
        ...
        cmd.add_argument("--bundle", help="Custom bundle name. Defaults to <site>-<mode>.")
    ...
    capture = subparsers.add_parser("capture", help="Open site, let user interact, save state files.")
